...
TEST(KeyExpanderTestCase, Aes128KeyTest)
{
    constexpr size_t ROUND_KEYS_AMOUNT = 11;
    std::array<uint8_t, aes::KEY128_SIZE> key =
            {0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};

    std::array<std::array<uint8_t, aes::BLOCK_SIZE>, ROUND_KEYS_AMOUNT> round_keys = {{
            {{0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c}},
    ...
    for (size_t i = 0; i < ROUND_KEYS_AMOUNT; ++i)
    {
        std::array<uint8_t, aes::BLOCK_SIZE> rk{0};
        gsl::span key_rk = exp_key.get_round_key(i);
        std::copy(key_rk.begin(), key_rk.end(), rk.begin());
        EXPECT_EQ(rk, round_keys[i]) << "Round key " << i << " failed.";
    }
}

TEST(KeyExpanderTestCase, Aes192KeyTest)
{
    constexpr size_t ROUND_KEYS_AMOUNT = 13;
    std::array<uint8_t, aes::KEY192_SIZE> key = {
        0x8E, 0x73, 0xB0, 0xF7, 0xDA, 0x0E, 0x64, 0x52, 0xC8, 0x10, 0xF3, 0x2B,
        0x80, 0x90, 0x79, 0xE5, 0x62, 0xF8, 0xEA, 0xD2, 0x52, 0x2C, 0x6B, 0x7B
    };

    std::array<std::array<uint8_t, aes::BLOCK_SIZE>, 13> round_keys = {{
